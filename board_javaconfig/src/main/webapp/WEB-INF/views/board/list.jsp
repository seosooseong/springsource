					...
					<tbody>
						...
						<c:forEach var="vo" items="${list}">
							<tr>
								...
								<!-- http://localhost:8081/board/(현재주소) -->
								<td>${vo.writer}</td>
								<td><fmt:formatDate value="${vo.regdate}" pattern="yyyy-MM-dd HH:mm" /></td>
								<td><fmt:formatDate value="${vo.updatedate}" pattern="yyyy-MM-dd HH:mm" /></td><!-- 패턴없으면 년월일(기본) -->
							</tr>
						</c:forEach>
					</tbody>
				</table>
						...
						<div class="col-md-8">
							...
							<form action="" id="searchForm">
								<select name="type" id="">
									...
									<option value="TC" <c:out value="${pageVO.cri.type=='TC'?'selected':''}"/>>제목or내용</option>
									<option value="TW" <c:out value="${pageVO.cri.type=='TW'?'selected':''}"/>>제목or작성자</option>
									<option value="TCW" <c:out value="${pageVO.cri.type=='TCW'?'selected':''}"/>>제목or내용or작성자</option>
								</select> 
				...
				<div class="text-center">
					<ul class="pagination">
						...
						<c:forEach var="idx" begin="${pageVO.startPage}"
							...
							<li
								class="paginate_button ${pageVO.cri.pageNum==idx?'active':''}"><a
								href="${idx}">${idx}</a>
						</c:forEach>
